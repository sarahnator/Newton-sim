"""CLI entry point for the Genesis robot-arm medium-viscosity reference variant.

Run from the ``genesis-sim`` conda env. This reference liquid sits between
the water and honey examples and is useful for calibration-demo slides.
"""
...
def _load_module():
    ...
    spec = importlib.util.spec_from_file_location("robotic_arm_pour_genesis_medium", path)
    assert spec and spec.loader
    module = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = module
    spec.loader.exec_module(module)
    module.configure_medium_variant()
    return module
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output-path",
        type=str,
        default="outputs/robotic_arm_pour_medium_genesis.mp4",
    )
    parser.add_argument(
        "--no-video",
        action="store_true",
    ...
    parser.add_argument(
        "--rebake",
        action="store_true",
        help="Rebuild the settled starting medium-liquid cache before running.",
    )
